...
from ultralytics.nn.tasks import (ClassificationModel, DetectionModel, PoseModel, ScrambledDetectionModel, ScrambledPoseModel, SegmentationModel,
...
TASK_MAP = {
    ...
    'scrambleddetect': [ScrambledDetectionModel, yolo.v8.detect.ScrambledDetectionTrainer, yolo.v8.detect.ScrambledDetectionValidator,
                      yolo.v8.detect.ScrambledDetectionPredictor],
    'scrambledpose': [ScrambledPoseModel, yolo.v8.pose.ScrambledPoseTrainer, yolo.v8.pose.ScrambledPoseValidator,
                      yolo.v8.pose.ScrambledPosePredictor]
    }
...
class YOLO:
    ...
    def train(self, **kwargs):
        ...
        if not overrides.get('resume') and not overrides.get('pretrained', False):  # manually set model only if not resuming
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model
        elif overrides.get('pretrained', False):
