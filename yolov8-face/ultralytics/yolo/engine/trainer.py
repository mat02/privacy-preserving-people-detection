...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        # Freeze layers
        if self.args.get('freeze_layers', None):
            from scrambled_train_utils import freeze_layers_inplace
            freeze_layers_inplace(self.model, self.args.freeze_layers, True)
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            # if RANK in (-1, 0):
            # Plot adaptation features
            # TODO extract as callback?
            model_modules_dict = {k: v for (k,v) in self.model.named_modules()}
            model_modules_names = [k for k in model_modules_dict.keys() if 'bs_matrix' in k]
            if len(model_modules_names) > 0:
                model_modules_names = model_modules_names[0].replace(".bs_matrix", "")
                if hasattr(model_modules_dict[model_modules_names], 'visualize_adaptation_network'):
                    model_modules_dict[model_modules_names].visualize_adaptation_network(epoch, self.save_dir / f"adaptnet_{RANK}")
    ...
    def save_model(self, target=""):
        ...
        if target:
            torch.save(ckpt, target, pickle_module=pickle)
