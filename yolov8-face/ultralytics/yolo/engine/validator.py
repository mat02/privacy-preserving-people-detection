...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            self.args.plots = trainer.stopper.possible_stop or (trainer.epoch == trainer.epochs - 1) or True # TODO parametrize?
            model.eval()
        else:
        ...
        for batch_i, batch in enumerate(bar):
            ...
            if self.args.plots and batch_i < 3:
                self.plot_val_samples(trainer.epoch if self.training else None, batch, batch_i)
                self.plot_predictions(trainer.epoch if self.training else None, batch, preds, batch_i)
