...
class PoseValidator(DetectionValidator):
    ...
    def plot_val_samples(self, epoch, batch, ni):
        """Plots and saves validation set samples with predicted bounding boxes and keypoints."""
        epoch = f"epoch{epoch}_" if epoch is not None else ""
        plot_images(batch['img'],
                    ...
                    paths=batch['im_file'],
                    fname=self.save_dir / f'val_{epoch}batch{ni}_labels.jpg',
                    names=self.names,
                    on_plot=self.on_plot)

    def plot_predictions(self, epoch, batch, preds, ni):
        ...
        plot_images(batch['img'],
                    *output_to_target(preds, max_det=self.args.max_det),
                    kpts=pred_kpts,
                    paths=batch['im_file'],
                    fname=self.save_dir / f'val_{epoch}batch{ni}_pred.jpg',
                    names=self.names,
...
class ScrambledPoseValidator(PoseValidator):
    def postprocess(self, preds):
        """Apply non-maximum suppression and return detections with high confidence scores."""
    ...
    #     from ultralytics.yolo.data.augment import Compose, Format, v8_transforms, LetterBox
    #     from ultralytics.yolo.utils import colorstr, ops
    #     from ultralytics.yolo.data.scrambler import ScramblerTransform
    #     if self.augment:
    ...
    #                               ScramblerTransform(
    #                                 p=1.0 if hyp.scr_enabled else 0.0,
    #                                 block_size=hyp.scr_block_size,
    #                                 num_blocks=hyp.scr_num_blocks,
    #                                 seed=hyp.scr_seed,
    #                                 channels=hyp.scr_channels,
    #                                 pixel_permutation=hyp.scr_pixel_permutation,
    #                                 block_permutation=hyp.scr_block_permutation,
    #                                 copy=True),
    #                             ])
    #     transforms.append(
